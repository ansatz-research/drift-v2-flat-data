...
async def load_and_save_data(pid='', url='https://api.mainnet-beta.solana.com'):
    ...
    all_userstats = await ch.program.account['UserStats'].all()

    for user_account in all_userstats:
        uu = user_account.account.__dict__
        ufees = pd.DataFrame([uu['fees'].__dict__], index=[0]).T
        ufees.index = ['userfees.'+x for x in ufees.index]
        uu.pop('padding')
        uu.pop('fees')
        df_1 = pd.DataFrame(uu, index=[0]).T
        df = pd.concat([df_1, ufees],axis=0)
        authority = str(df.loc['authority'].values[0])
        df.to_csv("data/userstats/%s.csv" % str(authority))
